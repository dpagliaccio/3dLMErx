...
#### minor edits made by David Pagliaccio (June 29, 2021) to allow for models with brain predicting clinical outcomes - noted with #DP EDIT
#### can now specify brain as predictor (labeled yy), e.g. -model outcome~age+sex+visit*yy+(1|Subj)
...
#################################################################################
##################### Begin 3dLMEx Input functions ################################
#################################################################################

#The help function for 3dLMEx batch (AFNI-style script mode)
...
------
 ...
 3dLMEx is a revised and advanced version of its older brother 3dLME in the sense
 that the former is much more flexible in specifying the random-effects components
 than the latter. Also, 3dLMEx uses the R package \'lme4\' while 3dLME was written
 ...
 flexibility of 3dLMEx lies in its adoption of random-effects notations by the R
 ...
 the 3dLMEx interface is largely similar to 3dLME except
 ...
 and the composite effects (automatically generated by 3dLMEx) are represented in the
 ...
 3dLMEx allows for the incorporation of various types of explanatory variables
 ...
 first before running 3dLMEx: "lmerTest", "phia" and "snow". To install these R
 ...
 Once the 3dLMEx command script is constructed, it can be run by copying and
...
read.LME.opts.batch <- function (args=NULL, verb = 0) {
  params <- list (
    ...
    '-dbgArgs' = apl(n=0, h = paste(
      ...
      "         file called .3dLMEx.dbg.AFNI.args in the current directory",
  ...
  if (is.null(ops))
    errex.AFNI('Error parsing arguments. See 3dLMEx -help for details.')
...
glfConstr <- function(cStr, dataStr) {
  ...
  varsOK <- vars %in% c("yy",colnames(dataStr)) #varsOK <- vars %in% colnames(dataStr) # # # # # # # # # # # # # # # DP EDIT - Now allow yy in GLT
  if(all(varsOK)) {
...
#Change options list to 3dLMEx variable list
...
if(!exists('.DBG_args')) {
  ...
  if ( '-dbgArgs' %in% args ) {
    try(save(args, rfile, file=".3dLMEx.dbg.AFNI.args", ascii = TRUE), silent=TRUE)
  }
} else {
...
#lop$model <- as.formula(paste('yy ~ ', lop$model)) # # # # # # # # # # # # # # # DP EDIT - removed this specification, specify yy as brain when building model  
require(lmerTest)
require(phia)
fm<-NULL
...
while(is.null(fm)) {
  ...
  if(!is.null(fm))  {
    ...
    ii<-xinit; jj <- yinit; kk <- kk+1 } else {
      ...
      cat('would cause grief for 3dLMEx.\n')
